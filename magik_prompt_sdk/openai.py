...
class OpenAI:
    ...
    def chat_completion(self, model, prompt):
        ...
        promptResponse = requests.post(
            "http://localhost:9000/api/v1/prompt", data={"text": prompt}).json()
    ...
    def save_prompt_run_chat_completion(self, model, completion, promptResponse):
        ...
        """
        requests.post("http://localhost:9000/api/v1/promptRun", json={"promptId": promptResponse["data"]["prompt"]["id"], "languageModelId": model,"promptSent": promptResponse["data"]["prompt"]["text"],"promptResponse": completion.choices[0].message.content, "tokensUsed": int(completion.usage.total_tokens)})
    ...
    def save_prompt_run_completion(self, model, completion, promptResponse):
        ...
        Save prompt run to the database
        """
        requests.post("http://localhost:9000/api/v1/promptRun", json={"promptId": promptResponse["data"]["prompt"]["id"], "languageModelId": model,"promptSent": promptResponse["data"]["prompt"]["text"],"promptResponse": completion.choices[0].text, "tokensUsed": int(completion.usage.total_tokens)})
